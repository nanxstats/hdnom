...
We split the samples into five risk groups. In practice, the number of
...
the number at risk from one year to six years for the five risk groups,
with the function `hdnom.kmplot()`:

```{r, fig.width = 8, fig.height = 8, out.width = 600, out.height = 600}
hdnom.kmplot(cal.int, group.name = c('Highest risk', 'High risk',
                                     'Medium risk', 'Low risk', 'Lowest risk'),
             time.at = 1:6 * 365)

hdnom.kmplot(cal.ext, group.name = c('Highest risk', 'High risk',
                                     'Medium risk', 'Low risk', 'Lowest risk'),
             time.at = 1:6 * 365)
```
...
type we want to compare. Adaptive lasso has slightly better performance
than lasso in this case, since its predicted overall survival
probabilities are generally closer to the observed survival probabilities.
...
We thank Prof. Matthew Stephens, Dr. Samuel Volchenboum, and Dr. Jorge Andrade
for their great suggestions. For further information about the project,
please visit: [http://hdnom.org](http://hdnom.org).
The hdnom web application: [http://hdnom.io](http://hdnom.io).
