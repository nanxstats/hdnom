---
title: "Nomograms for High-Dimensional Data"
author: "Miaozhu Li <<miaozhu.li@duke.edu>> <br>
...
nomogram plotting, model validation, model calibration, and model comparison.
...
parameter tuning is usually needed.
...
model validation, model calibration, and model comparison with `hdnom`.
...
and _pretend_ that they form an external validation dataset, then
...
the median of the observed survival probability estimated by Kaplan-Meier method
...
The exact $p$-values are stored as `cal.int.logrank$pval` and
`cal.ext.logrank$pval`.
...
as above. The model comparison functions in `hdnom` have a minimal input
...
performance. To continue the example, we still compare lasso and
adaptive lasso via calibration:
...
cmp.cal =
  hdnom.compare.calibrate(x, time, event,
                          model.type = c("lasso", "alasso"),
                          method = "cv", nfolds = 5,
                          pred.at = 365 * 9, ngroup = 3, seed = 1001,
                          trace = FALSE)

cmp.cal
summary(cmp.cal)
plot(cmp.cal, xlim = c(0.4, 1), ylim = c(0.4, 1))
```

The summary output and the plot show the calibration result for each model
...
We thank Prof. Matthew Stephens and Dr. Samuel Volchenboum for their
great suggestions that helped us improve the package quality.

For further information about the project, please visit:
[http://hdnom.org](http://hdnom.org).

The hdnom web application:
[http://hdnom.io](http://hdnom.io).
